...
type TenantSetting struct {
	...
	RefundApplyRange string `gorm:"size:64;not null;default:30" json:"refundApplyRange"`
	// ShippedRefundApplyRange 已发货退款/退款成功的申请时间：all / 7 / 30 / 90 / YYYY-MM-DD,YYYY-MM-DD。
	ShippedRefundApplyRange string `gorm:"size:64;not null;default:30" json:"shippedRefundApplyRange"`
	// ReturnRefundApplyRange 退货退款/退款成功的申请时间：all / 7 / 30 / 90 / YYYY-MM-DD,YYYY-MM-DD。
	ReturnRefundApplyRange string    `gorm:"size:64;not null;default:30" json:"returnRefundApplyRange"`
	CreatedAt              time.Time `json:"createdAt"`
	UpdatedAt              time.Time `json:"updatedAt"`
}
...
type ShippedRefundSuccess struct {
	ID                  uint64     `gorm:"primaryKey" json:"id"`
	TenantID            uint64     `gorm:"index;not null" json:"tenantId"`
	ShopID              uint64     `gorm:"uniqueIndex:uk_shop_shipped_refund;index;not null" json:"shopId"`
	PlatformAftersaleID string     `gorm:"size:64;uniqueIndex:uk_shop_shipped_refund;not null" json:"platformAftersaleId"`
	OrderNo             string     `gorm:"size:64;index" json:"orderNo"`
	ProductTitle        string     `gorm:"size:512" json:"productTitle"`
	ProductImage        string     `gorm:"size:2048" json:"productImage"`
	SKU                 string     `gorm:"size:256" json:"sku"`
