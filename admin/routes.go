...
func RegisterRoutes(
	...
	g.POST("/shops/from-agent", shopH.CreateFromAgent)
	g.GET("/agent-online-shops", shopH.ListOnlineAgentShops)
	...
	g.POST("/shops/:id/enable-agent-collect", shopH.EnableAgentCollect)
	...
	g.GET("/issue-records/lookup", shopH.IssueLookup)
	g.GET("/issue-records", shopH.ListIssues)
	g.POST("/issue-records", shopH.CreateIssue)
	g.GET("/issue-records/:id", shopH.GetIssue)
	g.PUT("/issue-records/:id", shopH.UpdateIssue)
	g.PATCH("/issue-records/:id/status", shopH.UpdateIssueStatus)
	g.DELETE("/issue-records/:id", shopH.DeleteIssue)
	g.GET("/shop-tickets", shopH.ShopTickets)
	g.GET("/nav-counts", shopH.NavCounts)
