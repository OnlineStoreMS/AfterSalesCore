...
func (h *ShopHandler) CreateFromAgent(c *gin.Context) {
	var in dto.ShopFromAgentInput
	if err := c.ShouldBindJSON(&in); err != nil {
		response.Fail(c, http.StatusBadRequest, err.Error())
		return
	}
	item, err := h.ss(c).CreateFromAgent(&in)
	if err != nil {
		httputil.HandleServiceError(c, err)
		return
	}
...
func (h *ShopHandler) ListOnlineAgentShops(c *gin.Context) {
	list, err := h.ss(c).ListOnlineAgentShops(c.Query("platform"))
	if err != nil {
		httputil.HandleServiceError(c, err)
		return
	}
...
func (h *ShopHandler) EnableAgentCollect(c *gin.Context) {
	id, err := httputil.ParseID(c)
	if err != nil {
		response.Fail(c, http.StatusBadRequest, "invalid id")
		return
	}
	item, err := h.ss(c).EnableAgentCollect(id)
	if err != nil {
		httputil.HandleServiceError(c, err)
		return
	}
