...
import {
  completeEdgeRecord,
  createEdgeRecord,
  uploadEdgeRecordPhoto,
  uploadEdgeRecordVideo,
  RECORD_TYPE_LABEL,
  type RecordType,
} from '../../api/edgeRecord'
...
const step = ref(0)
const stepTitles = ['识别单号', '录制视频', '拍摄照片', '确认提交']
...
function cameraUnavailableMessage(): string {
  if (!window.isSecureContext) {
    return '摄像头需要在 HTTPS 或 localhost 环境下使用，也可改用手动输入快递单号。'
  }
...
async function initCamera() {
  ...
  try {
    ...
    if (!hasLiveVideoTrack(media)) {
      ...
      cameraError.value = '未检测到摄像头画面，请检查系统与浏览器摄像头权限。'
      return
    }
    stream.value = media
    if (videoRef.value) {
...
function startRecording() {
  ...
  const recorder = mime
    ? new MediaRecorder(stream.value, { mimeType: mime, videoBitsPerSecond: 2_500_000 })
...
function capturePhoto() {
  ...
  canvas.toBlob((blob) => {
    if (!blob) return
    photos.value.push({ blob, previewUrl: URL.createObjectURL(blob) })
...
function goPrev() {
  ...
  if (step.value === 1 && recording.value) stopRecording()
  step.value -= 1
...
async function handleSubmit() {
  ...
  try {
    const record = await createEdgeRecord({
      type: recordType.value,
      trackingNo: trackingNo.value,
      remark: remark.value || undefined,
    })
    const videoFile = new File([videoBlob.value], `${recordType.value}-${trackingNo.value}.webm`, {
      type: videoBlob.value.type || 'video/webm',
    })
    await uploadEdgeRecordVideo(record.id, videoFile, recordDurationSec.value)
    for (const photo of photos.value) {
      const file = new File([photo.blob], `photo-${Date.now()}.jpg`, { type: 'image/jpeg' })
      await uploadEdgeRecordPhoto(record.id, file)
    }
    await completeEdgeRecord(record.id, { remark: remark.value || undefined })
    ElMessage.success(`${typeLabel.value}记录已提交`)
    router.push(`${listPath.value}/${record.id}`)
  } catch (e) {
...
<template>
  <div class="record-create">
    <div class="top-bar">
      ...
      <span class="title">录制{{ typeLabel }}</span>
      <el-tag type="info" size="small">云端浏览器 · edge: cloud</el-tag>
    </div>
    ...
    <el-card>
      <div class="camera-section">
        <div class="camera-wrap">
          <video ref="videoRef" autoplay playsinline muted class="camera-preview" :class="{ hidden: step === 3 && videoPreviewUrl }" />
          <video v-if="step >= 1 && videoPreviewUrl" :src="videoPreviewUrl" controls playsinline class="camera-preview overlay" />
        ...
        <div v-if="step === 0" class="panel">
          ...
          <el-input v-model="trackingNo" placeholder="快递单号" size="large" clearable @input="onTrackingInput" />
          <div class="actions">
        ...
        <div v-else-if="step === 1" class="panel">
          <p class="hint">请全程录制{{ typeLabel }}过程，确保包裹外观与内物清晰可见。</p>
          ...
          <div class="actions">
            <el-button v-if="!recording" type="danger" :icon="VideoCamera" @click="startRecording">
              {{ hasVideo ? '重新录制' : '开始录制' }}
            </el-button>
        ...
        <div v-else-if="step === 2" class="panel">
          ...
          <div v-if="photos.length" class="photo-list">
            <div v-for="(photo, idx) in photos" :key="idx" class="photo-card">
              <img :src="photo.previewUrl" alt="照片" />
        ...
        <div v-else class="panel">
          <el-descriptions :column="1" border>
            ...
            <el-descriptions-item label="照片">{{ photos.length }} 张</el-descriptions-item>
            <el-descriptions-item label="录制端">cloud（云端浏览器）</el-descriptions-item>
          </el-descriptions>
...
.camera-section { display: flex; flex-direction: column; gap: 20px; }
.camera-wrap { position: relative; background: #000; border-radius: 8px; overflow: hidden; max-width: 720px; aspect-ratio: 16 / 9; }
.camera-preview { width: 100%; height: 100%; object-fit: cover; display: block; }
.camera-preview.hidden { visibility: hidden; position: absolute; inset: 0; }
.camera-preview.overlay { position: absolute; inset: 0; visibility: visible; background: #000; }
.camera-placeholder { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; color: #fff; background: #1a1a1a; }
.camera-placeholder.error { color: #f56c6c; padding: 16px; text-align: center; }
.panel { max-width: 720px; }
.hint { color: #606266; margin: 0 0 12px; }
.actions { display: flex; flex-wrap: wrap; gap: 12px; align-items: center; margin-top: 12px; }
.record-status { margin-bottom: 4px; }
.photo-list { display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; margin-top: 16px; }
.photo-card { border: 1px solid #ebeef5; border-radius: 8px; padding: 8px; }
.photo-card img { width: 100%; height: 120px; object-fit: cover; border-radius: 4px; }
.remark-field { margin-top: 16px; }
</style>
